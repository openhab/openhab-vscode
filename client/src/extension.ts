...
import { OpenHABContentProvider } from "./ContentProvider/openHAB";

import { openBrowser, getSitemaps, openUI } from "./Utils";
...
async function init(disposables: Disposable[], config, context): Promise<void> {
  ...
  disposables.push(
    commands.registerCommand("openhab.basicUI", () => {
  ...
  disposables.push(
    commands.registerCommand("openhab.searchCommunity", (phrase?) => {
  ...
  disposables.push(
    commands.registerCommand("openhab.openKaraf", () => {
  ...
  disposables.push(
    commands.registerCommand("openhab.command.showInPaperUI", (query?) => {
  ...
  disposables.push(
    commands.registerCommand("openhab.command.things.docs", (query: Thing) => openBrowser(`https://www.openhab.org/addons/bindings/${query.binding}/`))
  );
  ...
  if (config.useRestApi) {
    ...
    disposables.push(
      commands.registerCommand("openhab.command.refreshEntry", () => {
        itemsExplorer.refresh();
        thingsExplorer.refresh();
      })
    );

    disposables.push(commands.registerCommand("openhab.command.copyName", query => ncp.copy(query.name || query.label)));

    disposables.push(commands.registerCommand("openhab.command.items.copyState", (query: Item) => ncp.copy(query.state)));

    disposables.push(
      commands.registerCommand("openhab.command.items.addRule", (query: Item) => {
    ...
    disposables.push(
      commands.registerCommand("openhab.command.items.addToSitemap", (query: Item) => {
    ...
    disposables.push(
      commands.registerCommand("openhab.command.things.addItems", (query: Thing | Channel) => {
        const itemsProvider = new ItemsProvider(query);
        itemsProvider.addToItems();
      })
    );

    disposables.push(commands.registerCommand("openhab.command.things.copyUID", query => ncp.copy(query.UID || query.uid)));
  }
...
export function activate(context: ExtensionContext) {
  ...
  init(disposables, config, context).catch(err => console.error(err));
}
